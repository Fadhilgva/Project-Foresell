        ...
        <div class="row">
            <div class="col-lg-10 mx-auto" style="margin-top: 60px; margin-bottom:50px">
                ...
                @error('phone') is-invalid @enderror"
                                            id="phone" name="phone" placeholder="Enter your phone phone"
                ...
                @error('city') is-invalid @enderror"
                                ...
                                <div class="col-md-6 my-1">
                                    <div class="form-floating-sm">
                                        ...
                                        <input type="text"
                                            class="form-control form-control-sm
                @error('postalcode') is-invalid @enderror"
                                            id="postalcode" name="postalcode" placeholder="Enter your post code"
                ...
                @error('address') is-invalid @enderror"
                                            id="address" name="address" placeholder="Enter your address" rows="3" required>{{ old('address', Auth()->user()->address) }}</textarea>
                                        @error('address')
                                            <div class="invalid-feedback">
                                                {{ $message }}
                                            </div>
                    ...
                    <div class="row">
                        <div class="col-lg-12 mx-auto" style="margin-top: 60px; margin-bottom:50px">
                            {{-- Order Navigasi --}}
                            {{-- <div>
                                <div class="h2 mb-5 text-center title">Billing Information</div>
                                <ul class="nav nav-tabs nav-fill border-bottom mb-3">
                                                    ...
                                                    @foreach ($banks->take(4) as $bank)
                                                        <div class="accordion-item">
                                                            <h2 class="accordion-header"
                                                                id="flush-heading{{ $bank->id }}">
                                                                <button class="accordion-button collapsed" type="button"
                                                                    data-bs-toggle="collapse"
                                                                    data-bs-target="#flush-collapse{{ $bank->id }}"
                                                                    aria-expanded="false"
                                                                    aria-controls="flush-collapse{{ $bank->id }}">
                                                                    @if ($bank->image)
                                                                        <img src="/image/admin/payment/{{ $bank->image }}"
                                                                            width="25" class="me-2">
                                                                    @else
                                                                        <img src="{{ asset('img/customer/cc.png') }}"
                                                                            width="25" class="me-2">
                                                                    @endif
                                                                    {{ $bank->name }}
                                                                </button>
                                                            </h2>
                                                            <div id="flush-collapse{{ $bank->id }}"
                                                                class="accordion-collapse collapse"
                                                                aria-labelledby="flush-heading{{ $bank->id }}"
                                                                data-bs-parent="#accordionFlushExample">
                                                                <div class="form-check my-5 mx-3">
                                                                    <input class="form-check-input" type="radio"
                                                                        name="bank" id="bank"
                                                                        value="{{ $bank->id }}" required>
                                                            ...
                                                            <th scope="row">
                                                                <label class="list-group-item d-flex gap-2 my-2">
                                                                    <input class="form-check-input flex-shrink-0"
                                                                        ...
                                                                        id="listGroupRadios1"
                                                                        value="{{ $courier->id }}" required />
                                                                    <span>
                                                                        @if ($courier->image)
                                                                            <img src="/image/admin/couriers/{{ $courier->image }}"
                                                                                class="rounded-circle me-1"
                                                                                width="30">
                                                                        @else
                                                                            <img src="{{ asset('img/customer/jnt.png') }}"
                                                                                class="rounded-circle me-1"
                                                                                width="30">
                                                                        @endif
                                                                        {{ $courier->name }}
                                                ...
                                                @foreach ($carts as $cart => $items)
                                                    @foreach ($items as $item)
                                                        <li class="d-flex align-items-center justify-content-between my-2">
                                                            <strong
                                                                class="small font-weight-bold">{{ $item->Product->name }}</strong>
                                                            <span
                                                                class="text-muted small">Rp{{ number_format($item->total_product, 0, ',', '.') }}
                                                                x {{ $item->qty }}
                                                            </span>
                                                        </li>
                                                    @endforeach
                            ...
                            <div>
                                <div class="row align-items-center text-center">
                                    <div class="col-md-6 mb-3 mb-md-0 text-md-start">
                                        <a class="btn btn-outline-dark" href="/shipping"><i
                                                class='bx bx-left-arrow-alt'></i>Back</a>
                                    </div>
                                    <div class="col-md-6 text-md-end">
                                        <button class="btn btn-dark">Place order <i
